...
os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")  = os.environ.get("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")","")
...
def call_claude(stats, a_report, b_report):
    if not os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")"):
        score = min(100, max(20, 20 + stats["engines_total"]*2 + stats["cycles"]))
        return {
            "health_score": score,
            "top_actions": [o.get("action","") for o in a_report.get("opportunities",[])[:3]],
            "synthesis": f"{stats['engines_total']} engines running, {stats['cycles']} autonomous cycles done. No API key - running on pure logic. First priority: add os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")") to GitHub Secrets to unlock full AI synthesis.",
            "next_run_priority": b_report.get("refined_priority","Add os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")") to GitHub Secrets"),
...
{{"health_score":<int 0-100>,"top_actions":["a1","a2","a3"],"synthesis":"one paragraph - what is actually happening and what to do","next_run_priority":"single most important thing","passive_income_progress":"honest assessment","meeko_headline":"punchy subject line phrase"}}"""
    try:
        r = requests.post("https://api.anthropic.com/v1/messages",
            headers={"x-api-key":os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")"),"Content-Type":"application/json","anthropic-version":"2023-06-01"},
