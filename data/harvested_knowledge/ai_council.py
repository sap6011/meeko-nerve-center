...
Required:
  os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")   — Claude API key (used for ANALYST + CHALLENGER fallback)
  GH_PAT / GITHUB_TOKEN
...
def call_analyst(context_str):
    if not os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")"):
        print("❌ os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")") missing — Analyst cannot run")
        sys.exit(1)
    print("  🔬 ANALYST reading system...")
    r = requests.post(
        "https://api.anthropic.com/v1/messages",
        headers={"x-api-key": os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")"),
