...
def extract_lessons(cycles, patterns):
    ...
    if trend == "improving":
        ...
        lessons.append({"lesson": "Health declining — check os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")")") is set in GitHub Secrets",
            "priority": "critical", "type": "action",
            "action": "Settings -> Secrets -> os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")")")"})
    ...
    for c in cycles[-3:]:
        if c.get("health_score", 100) < 50:
            lessons.append({"lesson": "Sub-50 health — API key 401. Check os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")")").",
                "priority": "critical", "type": "action"})
            break
    return lessons[:20]
