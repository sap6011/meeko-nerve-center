...
Uses os.getenv("os.getenv("ANTHROPIC_API_KEY")") (already in secrets) — no extra tokens.
...
def run():
    if not API_KEY:
        print("SKIP: os.getenv("os.getenv("ANTHROPIC_API_KEY")") not set")
        post_comment("❌ os.getenv("os.getenv("ANTHROPIC_API_KEY")") not configured in repo secrets.")
        sys.exit(0)
