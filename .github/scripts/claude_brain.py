...
def run():
    if not API_KEY:
        print("FATAL: os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")") not set")
        if TRIGGER_NUM != "0":
            post_comment(TRIGGER_NUM, "❌ `os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")` not in repo secrets.")
        sys.exit(0)
