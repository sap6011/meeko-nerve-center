...
ANTHROPIC_KEY = os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")
...
def call_claude(stats, a_report, b_report):
    if not os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")"):
        ...
        return {
            ...
            "synthesis": f"{stats['engines_total']} engines running, {stats['cycles']} autonomous cycles done. First priority: add os.getenv("os.getenv("ANTHROPIC_API_KEY")")",
            "next_run_priority": b_report.get("refined_priority", "Add os.getenv("os.getenv("ANTHROPIC_API_KEY")")"),
...
{{"health_score":<int 0-100>,"top_actions":["a1","a2","a3"],"synthesis":"one paragraph - what is actually happening and what to do","next_run_priority":"single most important thing","passive_income_progress":"honest assessment","meeko_headline":"punchy subject line phrase"}}"""
    try:
        r = requests.post("https://api.anthropic.com/v1/messages",
            headers={"x-api-key":os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")"), "anthropic-version":"2023-06-01", "Content-Type":"application/json"},
