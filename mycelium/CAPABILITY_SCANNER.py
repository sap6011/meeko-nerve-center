...
def scan():
    ...
    caps = {
        "anthropic_api": {
            "name": "Claude API",
            "status": "active" if os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")") else "blocked",
    ...
    priority = (
        "os.getenv("os.getenv("ANTHROPIC_API_KEY")")" if not os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")") else
