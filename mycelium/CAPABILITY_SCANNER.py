...
def scan():
    ...
    caps = {
        "anthropic_api": {
            "name": "Claude API",
            "status": "active" if os.environ.get("os.environ.get("os.environ.get("ANTHROPIC_API_KEY", "").strip()", "").strip()", "").strip() else "blocked",
            "impact": "SELF_BUILDER, KNOWLEDGE_WEAVER, REVENUE_OPTIMIZER, ARCHITECT all dead without this",
            "fix": "anthropic.com/console -> API Keys -> add os.environ.get("os.environ.get("ANTHROPIC_API_KEY", "").strip()", "").strip()",
        ...
        "huggingface": {
            ...
            "fix": "Add os.environ.get("os.environ.get("ANTHROPIC_API_KEY", "").strip()", "").strip()",
    ...
    priority = (
        "os.environ.get("os.environ.get("ANTHROPIC_API_KEY", "").strip()", "").strip()" if not os.environ.get("os.environ.get("os.environ.get("ANTHROPIC_API_KEY", "").strip()", "").strip()", "").strip() else
