...
def scan():
    ...
    caps = {
        "anthropic_api": {
            "name": "Claude API",
            "status": "active" if os.environ.get("os.getenv("os.getenv("ANTHROPIC_API_KEY")")", "").strip() else "blocked",
            "impact": "SELF_BUILDER, KNOWLEDGE_WEAVER, REVENUE_OPTIMIZER, ARCHITECT all dead without this",
            "fix": "anthropic.com/console -> API Keys -> add os.getenv("os.getenv("ANTHROPIC_API_KEY")")",
        ...
        "huggingface": {
            ...
            "fix": "Add os.getenv("os.getenv("ANTHROPIC_API_KEY")")",
    ...
    priority = (
        "os.getenv("os.getenv("ANTHROPIC_API_KEY")")" if not os.environ.get("os.getenv("os.getenv("ANTHROPIC_API_KEY")")", "").strip() else
