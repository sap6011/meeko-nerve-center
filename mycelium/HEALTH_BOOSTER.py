...
Health score (100 pts):
  20 — os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip() valid
...
SECRETS_NEEDED = {
    "os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()":    {"pts": 15, "category": "AI Brain", "how": "console.anthropic.com -> API Keys"},
...
def check_api_key():
    key = os.environ.get("os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()", "").strip()
    if not key:
        return False, "os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip() not set"
    try:
...
def score_and_report():
    ...
    if api_ok:
        ...
        fixes.append("Go to anthropic.com/console -> API Keys -> Create new key -> Add as os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()
