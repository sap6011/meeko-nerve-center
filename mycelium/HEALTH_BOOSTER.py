...
SECRETS_NEEDED = {
    "os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")":    {"pts": 15, "category": "AI Brain", "how": "console.anthropic.com -> API Keys"},
...
def check_api_key():
    key = os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")
    if not key:
...
def score_and_report():
    ...
    if api_ok:
        ...
        fixes.append("Go to anthropic.com/console -> API Keys -> Create new key -> Add as os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")
