...
def extract_lessons(cycles, patterns):
    ...
    if trend == "improving":
        ...
        lessons.append({"lesson": "Health declining -- check os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()",
            "priority": "critical", "type": "action",
            "action": "Settings -> Secrets -> os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()"})
    ...
    for c in cycles[-3:]:
        if c.get("health_score", 100) < 50:
            lessons.append({"lesson": "Sub-50 health -- check os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()",
                "priority": "critical", "type": "action"})
            break
    return lessons[:20]
