...
def _ask_anthropic(messages, max_tokens=2000, system=None):
    ...
    if not ANTHROPIC_KEY:
        raise RuntimeError("os.getenv("ANTHROPIC_API_KEY") not set")
...
def _write_wire_state():
    ...
    (DATA / "ai_client_state.json").write_text(json.dumps({"last_run": __import__("datetime").datetime.now().isoformat(), "status": "ok"}, indent=2), encoding="utf-8")
