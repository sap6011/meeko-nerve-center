...
def run():
    ...
    print(f"  Outreach sent this cycle: {sent_n} | Total all-time: {total}")

    # ── Phase 2: Crisis Handshakes ──────────────────────────────────────────
    # Read NGO handshake emails generated by CRISIS_MONITOR
    # These are pre-written, urgent humanitarian alerts
    handshake_file = DATA / "ngo_handshakes.json"
    crisis_sent = 0
    if handshake_file.exists() and GMAIL:
        try:
            hs_data = json.loads(handshake_file.read_text())
            handshakes = hs_data.get("handshakes", [])
            urgent = [h for h in handshakes if h.get("urgency") == "CRITICAL" and h.get("status") == "QUEUED"]
            print(f"\n  Crisis handshakes: {len(urgent)} CRITICAL queued")

            for h in urgent[:2]:  # Max 2 crisis emails per cycle
                if sent_n + crisis_sent >= 5:  # Hard cap total emails
                    break
                # Only send to orgs we have real contact info for
                # For now, log the handshake as ready — actual org emails
                # get added when we register with ReliefWeb / have org contacts
                org_key = h.get("org_key", "")
                print(f"  Crisis handshake ready: {org_key} — {h.get('subject','')[:60]}")
                h["status"] = "READY"
                crisis_sent += 1
            ...
            print(f"  Crisis handshake error: {e}")

    save_state(state)
    print(f"EMAIL_OUTREACH done. Outreach: {sent_n} | Crisis: {crisis_sent}")
