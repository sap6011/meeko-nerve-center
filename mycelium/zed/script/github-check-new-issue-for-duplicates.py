...
Environment variables:
    ...
    os.getenv("os.getenv("ANTHROPIC_API_KEY")")  - Anthropic API key for Claude
...
if __name__ == "__main__":
    ...
    anthropic_key = os.environ.get("os.getenv("ANTHROPIC_API_KEY")")

    if not github_token:
        log("Error: GITHUB_TOKEN not set")
        sys.exit(1)
    if not anthropic_key:
        log("Error: os.getenv("os.getenv("ANTHROPIC_API_KEY")") not set")
        sys.exit(1)
