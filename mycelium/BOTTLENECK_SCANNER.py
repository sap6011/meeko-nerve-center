...
def check_secrets():
    secrets = {
        "os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")":    {"impact": "CRITICAL", "unlocks": ["AI brain, self-builder, knowledge weaver"]},
...
def check_engines():
    ...
    for f in sorted(Path("mycelium").glob("*.py")):
        ...
        try:
            ...
            syntax_ok = False
        needs = [s for s in ["os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")",
...
def identify_bottlenecks(secrets, engines, gumroad, revenue):
    ...
    if anthropic and not anthropic["present"]:
        bottlenecks.append({
            ...
            "title": "os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")",
