...
API_KEY        = os.environ.get("os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()", "").strip()
...
def call_claude(prompt, heavy=False, conversation_history=None):
    if not API_KEY:
        return None, "os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip() not set"
...
def run_loop():
    ...
    if not API_KEY:
        log("WARNING: os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip() not set")
        log("  Windows: $env:os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip()='sk-ant-...'")
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions>
    ...
    if err and "error" in err.lower():
        ...
        print(f"   Add os.environ.get("os.getenv("ANTHROPIC_API_KEY")", "").strip() to env for full Claude brain")
