#!/usr/bin/env python3
"""
EXPERIMENT: fus_681bee93
TYPE:        FUSION
BORN:        2026-03-21T15:23:25.762770Z
PARENTS:     CRYPTO_BRIDGE  +  LEGACY_SIFTED_ultimate_ai_self_1
HYPOTHESIS:  data pipeline that writes live HTML

WHAT THIS DOES:
  Fuses the core logic of CRYPTO_BRIDGE and LEGACY_SIFTED_ultimate_ai_self_1 into one unified tool.
  CRYPTO_BRIDGE contributes: json_io, http_fetch, file_io, loop_engine, data_scoring
  LEGACY_SIFTED_ultimate_ai_self_1 contributes: json_io, http_fetch, file_io, git_ops, loop_engine
...
def load_knowledge():
    """Pull from CRYPTO_BRIDGE pattern -- read the knowledge base."""
...
def score_entries(entries: list) -> list:
    """Pull from LEGACY_SIFTED_ultimate_ai_self_1 pattern -- score + rank each entry."""
    scored = []
    for e in entries:
        s = 0
        # richness score
...
def fuse_and_report():
    ...
    out_html = EXPERIMENTS / "fusion_fus_681bee93.html"
    ...
    lines = [
        ...
        f"<h1>FUSION: fus_681bee93</h1>",
        f"<p style='color:#5a9e6f'>Born: {ts} | Parents: CRYPTO_BRIDGE + LEGACY_SIFTED_ultimate_ai_self_1</p>",
