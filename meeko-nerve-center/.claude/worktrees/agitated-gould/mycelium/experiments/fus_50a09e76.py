#!/usr/bin/env python3
"""
EXPERIMENT: fus_50a09e76
TYPE:        FUSION
BORN:        2026-03-21T15:23:25.920815Z
PARENTS:     CODE_ALCHEMIST  +  SWARM_TOOLBOX
HYPOTHESIS:  data pipeline that writes live HTML

WHAT THIS DOES:
  Fuses the core logic of CODE_ALCHEMIST and SWARM_TOOLBOX into one unified tool.
  CODE_ALCHEMIST contributes: json_io, http_fetch, file_io, git_ops, loop_engine
  SWARM_TOOLBOX contributes: json_io, http_fetch, file_io, git_ops, loop_engine
...
def score_entries(entries: list) -> list:
    """Pull from SWARM_TOOLBOX pattern -- score + rank each entry."""
    scored = []
    for e in entries:
        s = 0
        # richness score
...
def fuse_and_report():
    ...
    out_html = EXPERIMENTS / "fusion_fus_50a09e76.html"
    ...
    lines = [
        ...
        f"<h1>FUSION: fus_50a09e76</h1>",
        f"<p style='color:#5a9e6f'>Born: {ts} | Parents: CODE_ALCHEMIST + SWARM_TOOLBOX</p>",
