#!/usr/bin/env python3
"""
EXPERIMENT: fus_5c69183a
TYPE:        FUSION
BORN:        2026-03-21T15:20:11.240637Z
PARENTS:     CODE_ALCHEMIST  +  LEGACY_SIFTED_ultimate_ai_self
HYPOTHESIS:  data pipeline that writes live HTML

WHAT THIS DOES:
  Fuses the core logic of CODE_ALCHEMIST and LEGACY_SIFTED_ultimate_ai_self into one unified tool.
  CODE_ALCHEMIST contributes: json_io, http_fetch, file_io, git_ops, loop_engine
  LEGACY_SIFTED_ultimate_ai_self contributes: json_io, http_fetch, file_io, git_ops, loop_engine
...
def score_entries(entries: list) -> list:
    """Pull from LEGACY_SIFTED_ultimate_ai_self pattern -- score + rank each entry."""
    scored = []
    for e in entries:
        s = 0
        # richness score
...
def fuse_and_report():
    ...
    out_html = EXPERIMENTS / "fusion_fus_5c69183a.html"
    ...
    lines = [
        ...
        f"<h1>FUSION: fus_5c69183a</h1>",
        f"<p style='color:#5a9e6f'>Born: {ts} | Parents: CODE_ALCHEMIST + LEGACY_SIFTED_ultimate_ai_self</p>",
