#!/usr/bin/env python3
"""
EXPERIMENT: fus_bf5df3db
TYPE:        FUSION
BORN:        2026-03-21T15:20:11.387653Z
PARENTS:     CODE_ALCHEMIST  +  PROOF_OF_IMPACT
HYPOTHESIS:  data pipeline that writes live HTML

WHAT THIS DOES:
  Fuses the core logic of CODE_ALCHEMIST and PROOF_OF_IMPACT into one unified tool.
  CODE_ALCHEMIST contributes: json_io, http_fetch, file_io, git_ops, loop_engine
  PROOF_OF_IMPACT contributes: json_io, http_fetch, file_io, git_ops, data_scoring
...
def score_entries(entries: list) -> list:
    """Pull from PROOF_OF_IMPACT pattern -- score + rank each entry."""
    scored = []
    for e in entries:
        s = 0
        # richness score
...
def fuse_and_report():
    ...
    out_html = EXPERIMENTS / "fusion_fus_bf5df3db.html"
    ...
    lines = [
        ...
        f"<h1>FUSION: fus_bf5df3db</h1>",
        f"<p style='color:#5a9e6f'>Born: {ts} | Parents: CODE_ALCHEMIST + PROOF_OF_IMPACT</p>",
