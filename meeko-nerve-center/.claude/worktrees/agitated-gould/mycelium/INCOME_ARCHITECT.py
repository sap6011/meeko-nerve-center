...
def record_sale(amount=1.00):
    ...
    gaza_cut  = round(amount * 0.99, 2)
    meeko_cut = round(amount * 0.01, 2)
...
def check_auto_buy(state):
    ...
    state["total_to_gaza"] = round(state["total_to_gaza"] + 0.99, 2)
    state["balance"]       = round(state["balance"] + 0.01, 2)
    state["sales"].append({"timestamp": datetime.now().isoformat(), "amount": 1.00,
                           "gaza": 0.99, "meeko": 0.01, "source": "auto_loop"})
    save_loop_fund(state)
    print(f"  Auto-buy #{state['auto_buys']}. Gaza +$0.99. Loop: ${state['balance']:.2f}")
    return state, True
