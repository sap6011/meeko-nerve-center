...
def load_seed_topics():
    """Read omnibrain_seed.json to bias harvesting toward current priorities."""
    seed_keywords = []
    seed_subreddits = []
    seed_tags = []
    try:
        seed = json.loads(Path("data/omnibrain_seed.json").read_text())
        instructions = seed.get("instructions", [])
        insight = seed.get("key_insight", "")
        # Extract keywords from seed instructions and insight
        combined = " ".join(instructions + [insight]).lower()
        word_map = {
            "ward 8": ([], ["solarpunk", "communityplanning", "urbanism"], ["community"]),
            "grant": ([], ["grants", "nonprofit"], ["grants", "community"]),
            "cdbg": ([], ["communityplanning", "urbanism"], ["community"]),
            "revenue": ([], ["passive_income", "entrepreneur"], ["webdev", "productivity"]),
            "social": ([], ["socialmedia"], ["webdev"]),
            "art": ([], ["DigitalArt", "generative"], ["showdev"]),
            "mutual aid": ([], ["mutualaid", "solarpunk"], ["community"]),
            "content": ([], ["content_creation"], ["writing"]),
        }
        for keyword, (kw_list, subs, tags) in word_map.items():
            if keyword in combined:
                seed_subreddits.extend(subs)
                seed_tags.extend(tags)
        if seed_subreddits:
            print(f"  Seed biasing subreddits: {seed_subreddits[:3]}")
    except:
        pass
    return seed_subreddits[:4], seed_tags[:2]

def main():
    print("CONTENT_HARVESTER starting...")
    ts = datetime.now(timezone.utc).isoformat()
    seed_subs, seed_tags = load_seed_topics()
    base_subs = ["solarpunk", "Gaza", "technology", "ArtificialIntelligence"]
    merged_subs = list(dict.fromkeys(seed_subs + base_subs))[:5]  # seed-biased first, deduplicated
    base_tag = seed_tags[0] if seed_tags else "opensource"
    hn = fetch_hackernews(limit=5)
    reddit = fetch_reddit_json(subreddits=merged_subs)
    devto = fetch_devto(tag=base_tag, limit=4)
    ...
    harvest = {"timestamp": ts, "sources": {"hackernews": hn, "reddit": reddit, "devto": devto},
        ...
        "seed_bias": {"subreddits": seed_subs, "tags": seed_tags}}
