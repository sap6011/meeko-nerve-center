...
CORE_FILES = [
    ...
    ".github/workflows/AI_COUNCIL.yml",
    ...
    "mycelium/BRAVE_BRIDGE.py",
    "mycelium/GETSCREEN_BRIDGE.py",
]
# Keep only the N most recent save-state tags to prevent accumulation
MAX_SAVE_TAGS = 10
DATA_DIR = Path("data")
...
def _prune_old_tags():
    """Delete oldest save-state tags beyond MAX_SAVE_TAGS to prevent accumulation."""
    all_tags, _ = runcmd("git tag --sort=version:refname 2>/dev/null")
    save_tags = [t for t in all_tags.splitlines() if t.startswith("save-")]
    to_delete = save_tags[:-MAX_SAVE_TAGS] if len(save_tags) > MAX_SAVE_TAGS else []
    for t in to_delete:
        runcmd(f"git tag -d {t} 2>/dev/null || true")
        runcmd(f"git push origin --delete {t} 2>/dev/null || true")
    if to_delete:
        print(f"🧹 Pruned {len(to_delete)} old save tags (kept {MAX_SAVE_TAGS})")
