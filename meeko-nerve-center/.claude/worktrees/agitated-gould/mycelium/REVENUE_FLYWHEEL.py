...
STREAMS_INIT = {
    ...
    "corc_revenue":      {"name": "Ward 8 CORC Credits",   "type": "carbon_credits",   "balance": 0.0},
}
...
def run():
    ...
    # LOOP INTEGRATION: pull confirmed revenue from upstream chain outputs
    for chain_file, stream_key in [
        ("data/gumroad_state.json",        "gaza_rose_gallery"),
        ("data/revenue_audit.json",         None),  # audit has per-stream breakdowns
        ("data/kofi_state.json",            "digital_products"),
        ("data/first_sale_state.json",      "gaza_rose_gallery"),
    ]:
        fp = Path(chain_file)
        if not fp.exists(): continue
        try:
            obj = json.loads(fp.read_text(encoding="utf-8", errors="ignore"))
            if chain_file == "data/revenue_audit.json":
                for sk, val in obj.get("stream_balances", {}).items():
                    if sk in state["streams"]:
                        state["streams"][sk]["balance"] = max(
                            state["streams"][sk].get("balance", 0), float(val)
                        )
            elif stream_key:
                confirmed = float(obj.get("confirmed_revenue", obj.get("total_revenue", 0)))
                if confirmed > state["streams"].get(stream_key, {}).get("balance", 0):
                    state["streams"].setdefault(stream_key, STREAMS_INIT.get(stream_key, {}).copy())["balance"] = confirmed
        except Exception as e:
            print(f"Flywheel chain read error ({chain_file}): {e}")
