...
|-----------|-------------|
| Control PC | Windows 10/11 with internet |
...
| Google AI Studio | API key for Gemini | https://aistudio.google.com |
| Anthropic | API key for Claude (optional) | https://console.anthropic.com |
...
```
(Replace ROLE with: backend, frontend, tests, design, or utility)
...
### 6.2 CRITICAL: Restart Windows Tailscale
...
(Replace N with 1-6; AGENT_IP with actual IP; USER with username)
...
C:\Windows\System32\OpenSSH\ssh.exe pcN "git clone git@github.com:YOUR_USERNAME/ralph-loops-project.git ~/project"
```
...
## Requirements
- Fetch from http://100.x.x.1:3001/api/hello (replace with your backend IP)
...
# Check AI works (Claude or Gemini)
C:\Windows\System32\OpenSSH\ssh.exe pc1 "gemini -p 'Say hello' || claude -p 'Say hello'"
...
- `tasks/pending/design/` - Design agent tasks
- `tasks/pending/utility/` - Utility agent tasks
...
The `start-night.sh` script handles everything:
...
This will:
1. Run pre-flight checks (git, SSH, AI, tasks, scripts, sleep masking)
2. Auto-fix sleep masking if needed
3. Start workers on all agents
4. Start the manager (if ANTHROPIC_API_KEY is set)
...
- Validation results (passed/failed/none)
- Manager reviews and verdicts
- Items needing your attention
- Fix chain summary
- Skills metrics (if available)
- Recent commits
- Process status
...
| 9. Install Claude CLI | `cd ~ && npm install @anthropic-ai/claude-code` |
| 10. Install Gemini | `sudo npm install -g @google/gemini-cli` |
| 11. Set Gemini API key | `echo 'GEMINI_API_KEY=YOUR_KEY' \| sudo tee -a /etc/environment` |
| 12. Add BOTH SSH keys | Windows key + pc5 manager key |
| 13. Add to SSH config | Add Host entry on Windows AND pc5 |
| 14. Generate GitHub key | `ssh-keygen -t ed25519` then add to GitHub |
| 15. Clone repo | `git clone git@github.com:USER/REPO.git ~/project` |
| 16. Update config.json | Add new agent entry |
...
{
  "agents": {
    "pc1": { "name": "ralph-backend", "role": "backend", "ip": "100.x.x.1", "ssh": "pc1" },
    "pc2": { "name": "ralph-frontend", "role": "frontend", "ip": "100.x.x.2", "ssh": "pc2" },
    "pc3": { "name": "ralph-tests", "role": "tests", "ip": "100.x.x.3", "ssh": "pc3" },
    "pc4": { "name": "ralph-design", "role": "design", "ip": "100.x.x.4", "ssh": "pc4" },
    "pc5": { "name": "ralph-manager", "role": "manager", "ip": "100.x.x.5", "ssh": "pc5" },
    "pc6": { "name": "ralph-utility", "role": "utility", "ip": "100.x.x.6", "ssh": "pc6" }
  }
}
```
...
**Note:** Never delete oauth_creds.json - quota is tied to Google account!
...
### AI CLI
- Use API key from Google AI Studio for Gemini automation
...
- **Dual SSH keys**: Windows + pc5 manager need access to all workers
...
- Include validation blocks for better coverage (auto-validation is a fallback)

---

## 13. Script Reference

| Script | Version | Purpose | Usage |
|--------|---------|---------|-------|
| `worker.sh` | v11 | Main agent loop (auto-validation, dual AI) | `./worker.sh ROLE` |
| `safe-push.sh` | - | Git push with retries | Called by worker.sh |
| `start-night.sh` | v4.2 | Start all workers + manager (preflight with sleep fix) | `./start-night.sh` |
| `morning-review.sh` | v4.1 | Show overnight summary + metrics | `./morning-review.sh` |
| `manager-loop.sh` | - | Manager review loop | Runs on pc5 |
| `watchdog.sh` | - | Alert on inactivity | `./watchdog.sh` |
...
Fill in your own Tailscale IPs after setup:
...
```bash
# Start night run (does everything)
./scripts/start-night.sh

# Check status during the night
./scripts/status.sh
...
for pc in pc1 pc2 pc3 pc4 pc6; do ssh $pc "pkill -f worker.sh"; done
```
...
│   ├── done/ROLE/        ← Completed
│   ├── review/ROLE/      ← Needs human review
│   └── approved/ROLE/    ← Manager approved
...
*Ralph Loops v5.0 - 6-machine architecture with Claude primary, Gemini fallback, auto-validation*
