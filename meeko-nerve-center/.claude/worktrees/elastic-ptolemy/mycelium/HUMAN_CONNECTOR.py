...
API=os.environ.get(_ak, "")
PITCH="""SolarPunk is an open-source autonomous AI income system built by Meeko (MeekoThaRaccoon).

What it does:
- Generates AI art of Palestinian themes
- Sells it for $1 (Gaza Rose Gallery)
- 70c goes to Palestinian artists, 30c funds the next loop
...
2. Add Secrets (Settings -> Secrets -> Actions):
   - os.environ.get(_ak, "") (get from console.anthropic.com)
...
ONLY JSON: {{"intent":"curious/wants_to_help/wants_to_fork/wants_to_collaborate/wants_to_donate/press/researcher","tone":"warm/neutral/skeptical","summary":"1 sentence","build_something":true/false}}"""
    try:
        r=requests.post("https://api.anthropic.com/v1/messages",
            ...
            json={"model":"claude-sonnet-4-6","max_tokens":200,"messages":[{"role":"user","content":prompt}]},timeout=15)
...
ONLY the reply body text, no JSON."""
    try:
        r=requests.post("https://api.anthropic.com/v1/messages",
            headers={"x-api-key":API,"Content-Type":"application/json","anthropic-version":"2023-06-01"},
            json={"model":"claude-sonnet-4-6","max_tokens":400,"messages":[{"role":"user","content":prompt}]},timeout=20)
