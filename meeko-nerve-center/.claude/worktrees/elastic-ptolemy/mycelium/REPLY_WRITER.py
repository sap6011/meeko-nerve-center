...
def write_reply(org: str, their_text: str, parsed: dict) -> str:
    ...
    if not ANTHROPIC_KEY:
        return (
            ...
            f"[os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")")")")")")")")")") not set — add it to GitHub Secrets for auto-replies]\n\n"
            f"{SIG}{REPLY_FOOTER}"
        )
