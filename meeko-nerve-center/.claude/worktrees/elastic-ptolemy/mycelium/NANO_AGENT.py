...
class NanoAgent:
    ...
    def ask_claude(self, prompt: str, system: str = None, max_tokens: int = 2048) -> str:
        ...
        if not key:
            print("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("os.getenv("ANTHROPIC_API_KEY")")")")")")")")")")")")")")") not set -- skipping Claude call")
            return ""
        try:
            import anthropic
