...
def load_context():
    a_report, lessons, mutual_aid, compliance = {}, [], {}, {}
    for fname, target in [("neuron_a_report.json","a"),("lessons.json","l"),
                           ("mutual_aid_summary.json","m"),("compliance_status.json","c")]:
        fp = Path("data") / fname
        if fp.exists():
            try:
                obj = json.loads(fp.read_text())
                if target=="a": a_report=obj
                elif target=="l": lessons=obj if isinstance(obj,list) else []
                elif target=="m": mutual_aid=obj
                elif target=="c": compliance=obj
            except: pass
    if not a_report:
    ...
    community_resources = [o.get("description","") for o in mutual_aid.get("open_offers",[])[:3]]
    compliance_alerts = compliance.get("critical_count", 0)

    def _rule_based_skeptic():
        return {
...
Critical lessons from memory: {critical_lessons}
Community resources available: {community_resources}
Compliance alerts outstanding: {compliance_alerts}
Kill bad ideas. Refine good ones. Add missing risks. Flag anything the lessons say already failed.
    ...
    try:
        ...
        report = json.loads(text[s:e]) if s>=0 else {"vetted_opportunities":a_report.get("opportunities",[]),"risks":[]}
            ...
            if "401" in err or "403" in err or "unauthorized" in err.lower():
                print("  NEURON_B: Claude 401 — using rule-based fallback")
                report = _rule_based_skeptic()
            else:
                report = {"risks":[err],"vetted_opportunities":a_report.get("opportunities",[]),"refined_priority":a_report.get("priority_build","")}
