...
def load_context():
    engine_files = sorted([f.name for f in Path("mycelium").glob("*.py")]) if Path("mycelium").exists() else []
    data_files = sorted([f.name for f in Path("data").glob("*.json")]) if Path("data").exists() else []
    flywheel, loop_mem, synth_log, brain, kb = {}, [], {}, {}, {}
    lessons, cross_log = [], {}
    for fname in ["flywheel_state.json","loop_memory.json","synthesis_log.json","brain_state.json",
                  "consolidated_knowledge.json","lessons.json","cross_post_log.json"]:
        fp = Path("data") / fname
        if fp.exists():
            try:
                ...
                elif fname=="consolidated_knowledge.json": kb=obj
                elif fname=="lessons.json": lessons=obj if isinstance(obj,list) else []
                elif fname=="cross_post_log.json": cross_log=obj
            except: pass
    kb_insights = kb.get("insights", {})
    critical_lessons = [l["lesson"] for l in lessons if l.get("priority") in ("critical","high")]
    # LOOP INTEGRATION: read cycle_brief for current cycle context
    cycle_phase, cycle_focus, cycle_actions, last_cycle_summary = "PRE_REVENUE", "", [], ""
    cb = Path("data/cycle_brief.json")
    if cb.exists():
        try:
            brief = json.loads(cb.read_text())
            cycle_phase   = brief.get("phase", "PRE_REVENUE")
            cycle_focus   = brief.get("focus_this_cycle", "")
            cycle_actions = brief.get("top_actions", [])
            last_cycle_summary = brief.get("last_cycle", {}).get("summary", "")
        except: pass
    # Also pull loop_state for succeeded/failed engines
    loop_state = {}
    ls = Path("data/loop_state.json")
    if ls.exists():
        try: loop_state = json.loads(ls.read_text())
        except: pass
    return {
        ...
        "kb_patterns": kb_insights.get("patterns", []),
        "kb_next_build": kb_insights.get("engines_to_build", []),
        "kb_advice": kb_insights.get("advice", ""),
        "critical_lessons": critical_lessons[:3],
        "posts_sent": cross_log.get("total_posted", 0),
        "cycle_phase": cycle_phase,
        "cycle_focus": cycle_focus,
        "cycle_actions": cycle_actions[:3],
        "last_cycle_summary": last_cycle_summary,
        "failed_engines": loop_state.get("failed_engines", [])[:5],
        "timestamp": datetime.now(timezone.utc).isoformat()
    }

def _rule_based_fallback(state):
    return {
        "opportunities": [
...
Revenue: ${state['revenue']:.2f} | Gaza total: ${state['total_to_gaza']:.2f} | Cycles: {state['loop_cycles']} | Posts sent: {state['posts_sent']} | Last insight: {state['last_insight']}
Knowledge base patterns: {state['kb_patterns']}
KB recommends building: {state['kb_next_build']}
KB advice: {state['kb_advice']}
Critical lessons from history: {state['critical_lessons']}
...
"meeko_action_needed":"one thing only Meeko can do or null"}}"""
    ...
    try:
        ...
        # On 401/auth failure, use rule-based fallback rather than empty report
        err = str(ex)
        if "401" in err or "403" in err or "unauthorized" in err.lower():
            print("  NEURON_A: Claude 401 — using rule-based fallback")
